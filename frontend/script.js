...
// ---------- OLD CATEGORY DATA (kept for reference, but NOT used for menu) ----------
// We now build the menu dynamically from product brands.
// You can remove this if you want, but it's harmless.
...
const subcategoryIcons = {}; // Not used
...
// ---------- BRAND-BASED MENU (NEW) ----------
function buildMainMenu() {
    const mainMenu = document.getElementById('mainMenu');
    mainMenu.innerHTML = '';

    // Get unique brands from products (ignore empty/undefined)
    ...
    brands.forEach(brand => {
        ...
        // Simple icon – you can replace with brand-specific icons if you like
        brandDiv.innerHTML = `<img src="https://cdn-icons-png.flaticon.com/512/1055/1055685.png" style="width:20px;height:20px;" loading="lazy"> ${brand}`;
        brandDiv.onclick = () => selectBrand(brand);
        mainMenu.appendChild(brandDiv);
    });

    // Optional "Accessories" button (for products that are not phones)
...
function selectBrand(brand) {
    ...
    // Build submenu with models (if you have a 'model' field)
...
// ========== BULK UPLOAD MODAL HANDLER (unchanged) ==========
...
// ========== SEARCH, NAVIGATION, ETC. (unchanged) ==========
